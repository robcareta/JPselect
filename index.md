...
When farmers choose what to grow, that choice depends on characteristics
(soil quality, water access, farming experience) that also affect
productivity. Estimating a production function on the selected
sub-sample (e.g., just the vegetable growers) therefore mixes the
technological relationship between inputs and output with the
...
`JPselect` implements the three-step procedure of [Koundouri & Nauges
(2005)](https://www.jstor.org/stable/40987295) that disentangles these
two effects:
...
The headline of the paper (and of this package) is that **skipping the
Heckman correction can flip the sign or kill the significance** of
...
> [`simulate_kiti_data()`](https://robcareta.github.io/JPselect/reference/simulate_kiti_data.md),
> a **synthetic** 239-farm dataset whose marginals roughly match Table 1
> of Koundouri & Nauges (2005). It is intended for demonstrating the
> methodology only; it is **not** the original Cyprus data and is not
> meant to reproduce the paper’s exact point estimates. To use real
> data, pass any data frame with the appropriate columns to
...
`summary(fit)` prints all three stages of the procedure end-to-end: the
probit selection equation (Step 1), the linear-quadratic mean production
function with the Mill’s ratio (Step 2), and the with/without
selectivity comparison for the risk function (Step 3).
...
[`summary()`](https://rdrr.io/r/base/summary.html) also appends the same
interpretation block that `print(fit)` shows at the bottom.

### What `plot(fit)` looks like

![Risk-function coefficients: with vs. without selectivity
correction](reference/figures/README-risk-plot.png)
...
|----|----|----|
| 1\. Probit selection + Inverse Mills Ratio | [`estimate_selection()`](https://robcareta.github.io/JPselect/reference/estimate_selection.md) | probit model and IMR |
| 2\. Linear-quadratic mean production function with IMR (OLS + HC1 SEs = just-identified GMM) | [`estimate_mean_function()`](https://robcareta.github.io/JPselect/reference/estimate_mean_function.md) | coefficient table, residuals |
| 3\. Cobb-Douglas risk function: `log\|residuals\| ~ log(inputs)`, with full-pipeline bootstrap SEs | [`estimate_risk_function()`](https://robcareta.github.io/JPselect/reference/estimate_risk_function.md) | variance elasticities |
| End-to-end wrapper that also fits the “without selectivity” comparison | [`jp_fit()`](https://robcareta.github.io/JPselect/reference/jp_fit.md) | `jpfit` S3 object with `print` / `summary` / `plot` / `coef` methods |
| Export the four resulting tables | [`jp_export()`](https://robcareta.github.io/JPselect/reference/jp_export.md) | one `.xlsx` with 5 sheets, one `.tex` with 4 booktabs tables, or a folder of CSVs |
...
`plot(fit)` shows this comparison side-by-side per input, and
`print(fit)` flags inputs whose conclusion actually changes between
specifications.
